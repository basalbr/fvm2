...
//    dd($valorTotal);
    ...
    public function nfs2(Request $request)
    {
        $start = new Carbon('first day of last month');
        $end = new Carbon('last day of last month');
        $pagamentos = OrdemPagamento::where('referencia', 'mensalidade')->where('valor', '>', 0)->where('status', 'Paga')->whereBetween('vencimento', [$start, $end])->paginate(100);
        $nm_lote = 800 + OrdemPagamento::orderBy('id', 'desc')->first()->id + $pagamentos->currentPage();
        $rps = '0|2.03.01|' . $nm_lote . '|' . '28398863000188|' . '121537|' . $pagamentos->count() .'|'. chr(13) . chr(10);
        $valorTotal = 0.0;
        $qtdNotas = 0;
        foreach ($pagamentos as $pagamento) {
            /*@var OrdemPagamento $pagamento*/
        ...
        return response($rps)->withHeaders([
            'Content-Type' => 'text/plain',
            'Cache-Control' => 'no-store, no-cache',
            'Content-Disposition' => 'attachment; filename="28398863000188'.date('Ymdhis').'.txt',
        ]);
    }
...
//                    $empresa->usuario->notify(new ReajusteMensalidade($empresa->getMensalidadeAtual(), Mensalidade::calculateMonthlyPayment($empresa->getMensalidadeAtual())));
    ...
    public function getMunicipio($search)
    {
        $municipios = array(
        ...
        foreach ($municipios as $k => $municipio) {
            if (mb_strtolower($search) == mb_strtolower($municipio)) {
                return $k;
            }
        }
        return 4202404;
