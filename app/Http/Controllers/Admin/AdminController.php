...
class AdminController extends Controller
{
    ...
    {
        $initialDate = (new Carbon('first day of 6 months ago'))->format('Y-m-d');
        $usuarios = Usuario::where('created_at','>=',$initialDate)->groupBy(DB::Raw('YEAR(created_at)'), DB::Raw("MONTH(created_at)"))->orderBy('created_at', 'desc')->select(DB::Raw("COUNT(*) as qtde, MONTH(created_at) as mes, YEAR(created_at) as ano"))->limit(6)->get()->toArray();
        $usuariosData = [];
        $empresas = Empresa::where('created_at','>=',$initialDate)->groupBy(DB::Raw('YEAR(created_at)'), DB::Raw("MONTH(created_at)"))->orderBy('created_at', 'desc')->select(DB::Raw("COUNT(*) as qtde, MONTH(created_at) as mes, YEAR(created_at) as ano"))->limit(6)->get()->toArray();
        $empresasData = [];
        $aberturas = AberturaEmpresa::where('created_at','>=',$initialDate)->groupBy(DB::Raw('YEAR(created_at)'), DB::Raw("MONTH(created_at)"))->orderBy('created_at', 'desc')->select(DB::Raw("COUNT(*) as qtde, MONTH(created_at) as mes, YEAR(created_at) as ano"))->limit(6)->get()->toArray();
        $aberturasData = [];
        $alteracoes = Alteracao::where('created_at','>=',$initialDate)->groupBy(DB::Raw('YEAR(created_at)'), DB::Raw("MONTH(created_at)"))->orderBy('created_at', 'desc')->select(DB::Raw("COUNT(*) as qtde, MONTH(created_at) as mes, YEAR(created_at) as ano"))->limit(6)->get()->toArray();
        $alteracoesData = [];
        foreach (array_reverse($usuarios) as $usuario) {
            $usuariosData[] = [Carbon::createFromFormat('Y-n-d', $usuario['ano'] . '-' . $usuario['mes'] . '-01')->format('U') * 1000, $usuario['qtde']];
        }
        foreach (array_reverse($empresas) as $empresa) {
            $empresasData[] = [Carbon::createFromFormat('Y-n-d', $empresa['ano'] . '-' . $empresa['mes'] . '-01')->format('U') * 1000, $empresa['qtde']];
        }
        foreach (array_reverse($aberturas) as $abertura) {
            $aberturasData[] = [Carbon::createFromFormat('Y-n-d', $abertura['ano'] . '-' . $abertura['mes'] . '-01')->format('U') * 1000, $abertura['qtde']];
        }
