...
<div role="tabpanel" class="tab-pane active animated fadeIn" id="tab-geral">
    ...
    @include('dashboard.imposto_renda.new.components.upload', ['descricao'=>'Comprovante de Residência *', 'referencia'=>'comprovante_residencia'])
    <div class="col-xs-12">
        <div class="form-group">
            <label>Ocupação *</label>
            <select class="form-control" name="id_ir_tipo_ocupacao">
                @foreach($tiposOcupacao as $ocupacao)
                    <option value="{{$ocupacao->id}}">{{$ocupacao->descricao}}</option>
                @endforeach
            </select>
        </div>
    </div>
