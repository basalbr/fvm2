<div role="tabpanel" class="tab-pane" id="tab-modal-doacoes">
    <p class="alert-info alert" style="display: block"><strong>Doações Efetuadas:</strong> Enviar documento comprobatório de doações efetuadas pelo
        declarante no ano
        de {{$anoAnterior}}</p>
    @include('dashboard.imposto_renda.edit.components.doacao')
    ...
    @include('dashboard.imposto_renda.edit.components.select', ['descricao'=>'doação a partidos políticos', 'referencia'=>'doacoes_partidos_politicos', 'opcoes'=>$irDoacoesPoliticas])
    <div class="clearfix"></div>
</div>